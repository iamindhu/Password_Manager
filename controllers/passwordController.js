        ...
        jwt.verify(token, process.env.jwt_secret, async (err, decodedToken) => {
          if (err) {
            res.locals.user = null;
              ...
              let user = await User.findById(decodedToken.id);
              const event = 'Access Page Opened';
              user.history.push({event});
              user.save();
               res.render('accesspassword', {userobj:user,allpasswords: user.addpassword, decrypt:decrypt});  
          }
        });
      } else {
        res.redirect("/login");      
...
module.exports.passwordstrength_get = (req, res) => {
  const token = req.cookies.jwt;
  if (token) {
      jwt.verify(token,process.env.jwt_secret, async (err, decodedToken) => {
        if (err) {
          res.locals.user = null;
            ...
            let user = await User.findById(decodedToken.id);
             res.render('passwordstrength', {userobj:user,allpassword:user.addpassword, decrypt:decrypt});  
        }
      });
    } else {
      res.locals.user = null;
            ...
            if (err) {
                ...
                if(auth){
                  ...
                  user.updateOne({total_strength:ts},function(err,success){
                   ...
                   user.updateOne({total_strength:ts});
                  user.history.push({event});
                  user.save(); 
                  res.status(200).json({ status: true });  
                  }
        ...
        jwt.verify(token, process.env.jwt_secret, async (err, decodedToken) => {
          let user = await User.findById(decodedToken.id);
            if (err) {
               ...
               const event= 'User details Updated';
                 user.history.push({event});
                user.save();
